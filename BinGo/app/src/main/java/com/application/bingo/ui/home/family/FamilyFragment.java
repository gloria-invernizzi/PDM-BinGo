...
public class FamilyFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        familyViewModel.getError().observe(getViewLifecycleOwner(), error -> Toast.makeText(getContext(), error, Toast.LENGTH_SHORT).show());

        familyViewModel.getSuccessMessage().observe(getViewLifecycleOwner(), msg -> Toast.makeText(getContext(), msg, Toast.LENGTH_SHORT).show());

        // Listeners
        btnCreate.setOnClickListener(v -> {
            if (currentUserEmail != null) {
                Toast.makeText(getContext(), "Creazione in corso per: " + currentUserEmail, Toast.LENGTH_SHORT).show();
                familyViewModel.createFamily(currentUserEmail);
            } else {
                // Se arrivi qui, il problema è l'autenticazione
                Toast.makeText(getContext(), "ERRORE: Devi rifare il Login!", Toast.LENGTH_LONG).show();
            }
        });
